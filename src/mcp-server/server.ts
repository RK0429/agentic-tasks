...
export function createMcpServer(options: CreateMcpServerOptions = {}): {
  ...
  registerTool(
    ...
    {
      stale_session_ids: z.array(z.string()).min(1),
      reason: z.enum(['heartbeat_timeout', 'context_overflow', 'manual_cleanup']),
      agent_id: z.string(),
    },
    (input) =>
      runtime.stale_lock_cleanup({
        stale_session_ids: input.stale_session_ids as string[],
        reason: input.reason as 'heartbeat_timeout' | 'context_overflow' | 'manual_cleanup',
        agent_id: String(input.agent_id),
      }),
  );
